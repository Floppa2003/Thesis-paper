...
int main()
{
    ...
    // Количество пар сэмплов
    const int num_pairs = 500000;

    // Открываем файл для записи результатов
    std::ofstream result_file("../data/random_sampling_stratified_continuous_mu.csv");
    ...
    std::vector<Sample> inverted_samples(num_pairs);
    ...
    for (int n = 1; n <= max_n; n++)
    {
        ...
        if (n > 1 && is_prime[n])
        {
            ...
            {
                // Пропускаем недействительные сэмплы
                if (!original_samples[i].is_valid && !inverted_samples[i].is_valid)
                {
                    continue;
                }
                ...
                // Обновляем инвертированный сэмпл
                if (inverted_samples[i].is_valid)
                {
                    // Расширяем массив, если нужно
                    if (inverted_samples[i].prime_values.size() <= idx)
                    {
                        inverted_samples[i].prime_values.resize(idx + 1);
                    }

                    inverted_samples[i].prime_values[idx] = -value; // Противоположное значение
                    inverted_samples[i].current_sum += (-value);
                    if (inverted_samples[i].current_sum < 0)
                    {
                        inverted_samples[i].is_valid = false;
                    }
                }
            }
        }
        ...
        else if (n > 1)
        {
            for (int i = 0; i < num_pairs; i++)
            {
                ...
                if (original_samples[i].is_valid)
                {
                    ...
                    for (int p : primes)
                    {
                        ...
                        if (num % p == 0)
                        {
                            ...
                            // Если степень = 1, умножаем на значение простого числа
                            if (power == 1)
                            {
                ...
                // То же для инвертированного сэмпла
                if (inverted_samples[i].is_valid)
                {
                    int value = 1;
                    int num = n;
                    ...
                    for (int p : primes)
                    {
                        ...
                        if (num % p == 0)
                        {
                            ...
                            {
                                int idx = prime_value_index[p];
                                value *= inverted_samples[i].prime_values[idx];
                            }
                        }
                    }
        ...
        double probability = static_cast<double>(original_success + inverted_success) / (2 * num_pairs);
        double theoretical = (n > 1) ? 1.0 / std::sqrt(std::log(n)) : 1.0;
        double ratio = (n > 1) ? probability * std::sqrt(std::log(n)) : probability;

        // Записываем результаты в файл
        result_file << n << "," << std::fixed << std::setprecision(8)
        ...
        if (n % 50000 == 0 || n == 1 || n == 2)
        {
            ...
            std::cout << "Inverted success rate: " << static_cast<double>(inverted_success) / num_pairs << std::endl;
    ...
    result_file.close();
    std::cout << "Computation complete! Results saved to random_sampling_stratified_continuous_mu.csv" << std::endl;
